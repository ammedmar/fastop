...
def test_reference_universal_operation_for_cp3_generator():
    universal = universal_operation(**_universal_kwargs(p=3, r=1, source_degree=2))
...
def test_native_universal_top_reduced_power_matches_oddp_oracle(
    ...
    assert native_universal_operation(**_universal_kwargs(p, r, source_degree)) == (
        universal_operation_oddp(**operation)
    )


def test_native_bockstein_slice_matches_oddp_oracle():
...
def test_precomputed_universal_terms_match_oddp_oracle(
    ...
    assert precomputed_terms(
        ...
        source_degree=source_degree,
        bockstein=bockstein,
    ) == universal_operation_oddp(**operation).terms
...
def test_native_uncataloged_formulas_match_oddp_oracle(
    p,
    r,
    source_degree,
    bockstein,
    ...
    oracle = universal_operation_oddp(
        **_bridge_kwargs(p, r, source_degree, bockstein)
    )
